...
@pytest.mark.parametrize("create_driver", ["env.login_url"], indirect=True)
def test_login_via_invalid_user_name(create_driver, login_page, env):
    invalid_user_name, password = env.invalid_login, env.password

    login_page.set_login(invalid_user_name).set_password(password).click_login_button_login_page()
...
@pytest.mark.parametrize("create_driver", ["env.login_url"], indirect=True)
def test_login_via_invalid_password(create_driver, login_page, env):
    user_name, invalid_password = env.login, env.invalid_password

    login_page.set_login(user_name).set_password(invalid_password).click_login_button_login_page()
...
@pytest.mark.parametrize("create_driver", ["env.login_url"], indirect=True)
def test_login_page_links_clickable(create_driver, login_page, env):
...
@pytest.mark.parametrize("create_driver", ["env.login_url"], indirect=True)
def test_login_data_in_elements(create_driver, login_page, env):
    user_name, password = env.login, env.password
